...
export function describeMove(event: ReversibleEvent): string {
  ...
  if (event.type === 'notes/basic-filled') return 'Filled basic candidates';
  ...
  const entries: GameLogEntry[] = events
    ...
    .map((event) => {
      ...
      if (event.type === 'game/imported') {
        const text = event.payload.importKind === 'camera-photo'
          ? 'Imported photo puzzle'
          : event.payload.importKind === 'progress-transfer'
      ...
      if (event.type === 'move/undone' || event.type === 'move/redone') {
        ...
        const targetText = target && (target.type === 'cell/value-entered' || target.type === 'cell/value-erased' || target.type === 'cell/note-toggled' || target.type === 'cell/notes-filled' || target.type === 'notes/basic-filled' || target.type === 'cell/cleared' || target.type === 'hint/revealed' || target.type === 'game/restarted')
          ? describeMove(target)
