...
export async function validateSharedPuzzle(payload: string, options: { walkthrough?: boolean } = {}): Promise<SharedPuzzleValidation> {
  ...
  const clueCount = grid.filter(Boolean).length;
  if (cages && options.walkthrough && work.length === 0) {
...
export function puzzleUrl(
  ...
  url.search = '';
  url.hash = '';
  url.searchParams.set('p', encodeSharedPuzzlePayload(givens, work, metadata, puzzle));
  return url.toString();
}
